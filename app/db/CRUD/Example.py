...
class ExampleCRUD:
    @staticmethod
    async def create_example(session: AsyncSession, example_data: dict) -> app.db.schemas.Example:
    ...
    async def get_example_by_id(session: AsyncSession, example_id: int) -> app.db.schemas.models:
        stmt = select(app.db.schemas.Example).where(app.db.schemas.models.id == example_id)
        result = await session.execute(stmt)
